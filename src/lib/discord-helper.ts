...
export class Discord {
	...
	public async get_presence(): Promise<DiscordPresence | DiscordError> {
		...
		let activities: Array<Activity> = data.activities.filter((activity: any) => {
			return activity.name === "Spotify" && activity.name === "Visual Studio Code"
		})
		...
		if (activities.length === 1) {
			...
			presence = "Listening to Spotify and Coding in VSCode"
		}
